    ...
    <div class="flex-1 flex flex-col h-full min-w-0">
        <header class="flex items-center justify-between whitespace-nowrap border-b border-slate-200 dark:border-slate-800 bg-surface-light dark:bg-surface-dark px-8 py-4 z-10">
            <div class="flex items-center gap-4">
                <h2 class="text-slate-900 dark:text-white text-xl font-bold leading-tight tracking-tight">Edit Ticket Orders</h2>
            </div>
...
@else
    ...
    @include('head')
    @section('title', 'Edit Ticket Orders ')
@endif
...
<main class="flex-1 flex flex-col items-center">
    <section class="w-full max-w-[720px] px-4 md:px-10 py-10">
        ...
        <div class="bg-white rounded-xl shadow p-8">
            <form method="POST" action="{{ route('ticket-orders.update', $order->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label class="block text-sm font-bold mb-1">Cancel Reason (if cancelled)</label>
                    <input
                        type="text"
                        name="cancel_reason"
