...
@section('content')
    <div class="w-full max-w-4xl px-4 py-10 space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-[#111318] dark:text-white">Edit Facility</h1>
                <p class="text-sm text-[#616f89] dark:text-gray-400">Update facility details or toggle its activity.</p>
            </div>
            ...
            <form action="{{ route('admin.facilities.update', $facility) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="capacity" value="{{ old('capacity', $facility->capacity) }}" min="1" required
                            class="w-full rounded-lg border border-[#e5e7eb] dark:border-[#2a3441] bg-white dark:bg-[#111827] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="number_of_venues" value="{{ old('number_of_venues', $venueCount) }}" min="1" max="100" required
                        class="w-full rounded-lg border border-[#e5e7eb] dark:border-[#2a3441] bg-white dark:bg-[#111827] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary" />
                    <p class="text-xs text-[#616f89] mt-1">Adjusting this will create or delete venues in this facility group.</p>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-[#111318] dark:text-gray-200 mb-1">Description</label>
                    <textarea name="description" rows="3"
                        class="w-full rounded-lg border border-[#e5e7eb] dark:border-[#2a3441] bg-white dark:bg-[#111827] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">{{ old('description', $facility->description) }}</textarea>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    @if ($facility->facility_image_path)
                        <img src="{{ asset('storage/' . $facility->facility_image_path) }}" alt="Facility image" class="w-48 h-32 object-cover rounded border border-[#e5e7eb] dark:border-[#2a3441]">
                    @else
                        <p class="text-sm text-[#616f89] dark:text-gray-400">No facility image uploaded.</p>
                    @endif
