    ...
    <div class="flex-1 flex flex-col h-full min-w-0">
        <header class="flex items-center justify-between whitespace-nowrap border-b border-slate-200 dark:border-slate-800 bg-surface-light dark:bg-surface-dark px-8 py-4 z-10">
            <div class="flex items-center gap-4">
                <h2 class="text-slate-900 dark:text-white text-xl font-bold leading-tight tracking-tight">Create Event Ticket </h2>
            </div>
        ...
        @include('head')
        @section('title', 'Create Event Tickets')
        @endif

        <main class="flex-1 flex flex-col items-center bg-[#f8fafc]">
            <section class="w-full max-w-[800px] px-4 md:px-10 py-10">

                <!-- Header -->
                <div class="mb-8 text-center">
                    <h1 class="text-3xl font-black text-[#111318]">
                        Ticket Setup
                    </h1>
                    <p class="text-[#616f89] mt-2">
                        Event:
                        <span
                            id="event-name"
                            class="font-semibold"
                ...
                <form method="POST" action="{{ route('event-tickets.store') }}">
                    ...
                    <!-- Ticket Cards -->
                    <div id="tickets-wrapper" class="space-y-6">

                        <!-- Ticket Card -->
                        <div class="ticket-card bg-white rounded-xl border border-[#dbdfe6] shadow-sm p-6 relative">

                            <div class="flex justify-between items-center mb-4">
                                <h3 class="text-lg font-bold text-[#111318]">
                                    Ticket #1
                                </h3>

                                <button
                                    type="button"
                                    class="remove-ticket hidden text-red-500 hover:text-red-700 transition"
                                    title="Remove ticket">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        name="tickets[0][ticket_name]"
                                        placeholder="Early Bird / Normal / VIP"
                                        value="{{ old('tickets.0.ticket_name') }}"
                                        class="w-full rounded-lg border px-4 py-2 focus:ring focus:ring-primary/20"
                                        required>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        type="number"
                                        name="tickets[0][total_quantity]"
                                        value="{{ old('tickets.0.total_quantity') }}"
                                        class="w-full rounded-lg border px-4 py-2"
                                        required>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="tickets[0][status]" class="w-full rounded-lg border px-4 py-2">
                                        @foreach (['draft','active','paused'] as $status)
                                        <option value="{{ $status }}"
                                            @selected(old('tickets.0.status')===$status)>
                                            {{ ucfirst($status) }}
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input
                                        type="datetime-local"
                                        name="tickets[0][sales_start_at]"
                                        value="{{ old('tickets.0.sales_start_at') }}"
                                        class="w-full rounded-lg border px-4 py-2"
                                        required>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        type="datetime-local"
                                        name="tickets[0][sales_end_at]"
                                        value="{{ old('tickets.0.sales_end_at') }}"
                                        class="w-full rounded-lg border px-4 py-2"
                                        required>
                                </div>
                    ...
                    <div class="flex justify-between items-center mt-6">
                        ...
                        <span class="text-xs text-[#616f89]">
                            You can create multiple ticket tiers
                        </span>
                    </div>
                    ...
                    <div class="mt-10">
                        <button
                            type="submit"
                            class="w-full h-12 rounded-xl bg-primary text-white font-bold text-lg hover:bg-blue-700 transition shadow">
                            Save Tickets
                        </button>
                    </div>
        ...
        <script>
            ...
            document.getElementById('add-ticket').addEventListener('click', () => {
                ...
                clone.querySelector('h3').innerText = `Ticket #${ticketIndex + 1}`;

                clone.querySelectorAll('input, select').forEach(el => {
                    el.name = el.name.replace(/\d+/, ticketIndex);
                    el.value = '';
                });

                const removeBtn = clone.querySelector('.remove-ticket');
