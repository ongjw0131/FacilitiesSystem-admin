@include('../login_head')
@section('title', 'Forgot Password - University Event Manager')
    ...
    <div class="flex min-h-screen w-full animate-fade-in">
        <div class="flex flex-col w-full lg:w-1/2 xl:w-5/12 p-8 sm:p-12 lg:p-16 justify-between relative bg-white dark:bg-gray-900 z-10 shadow-xl">
            <div class="flex items-center justify-between">
                ...
                <a class="flex items-center gap-1 text-sm font-medium text-text-muted hover:text-primary transition-colors cursor-pointer" onclick="history.back()">
            ...
            <div class="w-full max-w-md mx-auto space-y-8 animate-fade-in-up">
                <div class="flex flex-col gap-3">
                    ...
                    <h1 class="text-text-main dark:text-white text-3xl sm:text-4xl font-black leading-tight tracking-[-0.033em]">Forgot password?</h1>
                    <p class="text-text-muted dark:text-gray-400 text-base font-normal leading-normal">
                        No worries, we'll send you reset instructions.
                    </p>
                </div>
                <form method="POST" action="{{ route('password.request') }}" class="flex flex-col gap-6">
                    @csrf
                    <div class="flex flex-col gap-2">
                        <label class="text-text-main dark:text-gray-200 text-sm font-bold leading-normal" for="email">
                        ...
                        <div class="relative">
                            ...
                            <input class="form-input flex w-full min-w-0 resize-none overflow-hidden rounded-lg text-text-main dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/20 border border-[#dbdfe6] dark:border-gray-700 bg-white dark:bg-gray-800 focus:border-primary h-14 placeholder:text-text-muted pl-12 pr-4 text-base font-normal leading-normal transition-all" id="email" name="email" placeholder="example@student.tarc.edu.my" type="email" value="{{ old('email') }}" required />
                        </div>
                        @error('email')
                    ...
                    <div class="text-center pt-2">
                        <a class="text-text-muted dark:text-gray-400 hover:text-primary dark:hover:text-primary-light text-sm font-medium leading-normal flex items-center justify-center gap-2 group transition-colors" href="{{ route('user.login') }}">
                            Remember your password?
                            <span class="text-primary group-hover:underline font-semibold">Log in</span>
                        </a>
                    </div>
                </form>
            </div>
        ...
        <div class="hidden lg:flex lg:w-1/2 xl:w-7/12 relative bg-background-light dark:bg-gray-800 overflow-hidden">
            ...
            <div class="absolute bottom-0 left-0 p-16 w-full text-white z-20">
                <div class="max-w-lg space-y-6">
                    ...
                    <h2 class="text-4xl font-bold leading-tight">Managing campus life has never been simpler.</h2>
                    <p class="text-lg text-white/80 leading-relaxed">Secure access to all your society events, room bookings, and campus activities in one place.</p>
